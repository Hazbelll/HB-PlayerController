...
function hb_playercontroller.endControlCL()
	...
	if (ply.hb_playercontroller.plyViewRestore) then
		...
		if (IsValid(ctrld)) then
			local col = ply.hb_playercontroller.plyViewRestore["GetColor"]
			
			ctrld:SetColor(Color(col.r, col.g, col.b, 255))
			ctrld:SetRenderMode(ply.hb_playercontroller.plyViewRestore["GetRenderMode"])
		end
	end
...
function hb_playercontroller.overrideView(ply, pos, ang, fov)
	...
	if (ply.hb_playercontroller["plyView"] <= 100) and (GetViewEntity() == ply) then
		if not (ply.hb_playercontroller.plyViewRestore) then
			ply.hb_playercontroller.plyViewRestore = {
				["GetRenderMode"] = ctrld:GetRenderMode()
			}
		end
		ply.hb_playercontroller.plyViewRestore["GetColor"] = ctrld:GetColor()
		local col = ply.hb_playercontroller.plyViewRestore["GetColor"]
		
		ctrld:SetColor(Color(col.r, col.g, col.b, 0))
		ctrld:SetRenderMode(RENDERMODE_TRANSALPHA)
...
function hb_playercontroller.controllerHUD()
	...
	function ply.hb_playercontroller.controllerHUDPlyWeaponRefresh()
		...
		if not (ply.hb_playercontroller.plyMenu.menuWeapon:IsMenuOpen()) then
			...
			for k, v in pairs(ctrld:GetWeapons()) do
	...
	if (gmd.IsSandboxDerived) then
		...
		function ply.hb_playercontroller.controllerHUDPlySpawnsRefresh()
			...
			if (ply.hb_playercontroller["entTYPES"]) then
				...
				for k, v in pairs(ply.hb_playercontroller["entTYPES"]) do
